...
export async function sendVerificationEmail(email: string, code: string) {
  ...
  await transporter.sendMail({
    ...
    html: `<div style="font-family:Arial,sans-serif;max-width:520px;margin:auto;padding:32px;color:#17211b"><div style="font-size:13px;font-weight:700;letter-spacing:3px;color:#176b43">SECUREPATH SHIELD</div><h1 style="font-size:28px;margin:28px 0 10px">Verify your email</h1><p style="color:#66736b;line-height:1.7">Use this verification code to finish creating your SecurePath Shield account:</p><div style="margin:26px 0;padding:20px;text-align:center;background:#edf5ef;border-radius:8px;font-size:32px;font-weight:700;letter-spacing:10px;color:#125636">${code}</div><p style="color:#66736b;font-size:13px;line-height:1.7">This code expires in 10 minutes. Never share it with anyone.</p></div>`,
  });
}
...
export async function sendAccountReadyEmail(email: string) {
  ...
  await transporter.sendMail({
    ...
    html: `<div style="font-family:Arial,sans-serif;max-width:520px;margin:auto;padding:32px;color:#17211b"><div style="font-size:13px;font-weight:700;letter-spacing:3px;color:#176b43">SECUREPATH SHIELD</div><h1 style="font-size:28px;margin:28px 0 10px">Your email is verified</h1><p style="color:#66736b;line-height:1.7">Your secure banking account is ready. You can now sign in and manage your SecurePath Shield account.</p><a href="${process.env.APP_URL || "http://localhost:3000"}/login" style="display:inline-block;margin:22px 0;padding:14px 22px;border-radius:6px;background:#176b43;color:white;text-decoration:none;font-weight:700">Sign in to SecurePath Shield</a></div>`,
  });
}
...
export async function sendPasswordResetEmail(email: string, token: string) {
  ...
  await transporter.sendMail({
    ...
    html: `<div style="font-family:Arial,sans-serif;max-width:520px;margin:auto;padding:32px;color:#17211b"><div style="font-size:13px;font-weight:700;letter-spacing:3px;color:#176b43">SECUREPATH SHIELD</div><h1 style="font-size:28px;margin:28px 0 10px">Reset your password</h1><p style="color:#66736b;line-height:1.7">Use the secure button below to choose a new password. This link expires in 30 minutes.</p><a href="${url}" style="display:inline-block;margin:22px 0;padding:14px 22px;border-radius:6px;background:#176b43;color:white;text-decoration:none;font-weight:700">Reset password</a><p style="color:#66736b;font-size:13px">If you did not request this, you can safely ignore this email.</p></div>`,
  });
}
...
export async function sendCampaignEmail({
  ...
  await transporter.sendMail({
    ...
    html: `<div style="font-family:Arial,sans-serif;max-width:540px;margin:auto;padding:32px;color:#17211b"><div style="font-size:13px;font-weight:700;letter-spacing:3px;color:#176b43">SECUREPATH SHIELD</div><h1 style="font-size:27px;margin:28px 0 12px">${safeTitle}</h1><p style="color:#66736b;line-height:1.7">Hello ${safeName},</p><p style="color:#66736b;line-height:1.7">${safeMessage}</p><a href="${destination}" style="display:inline-block;margin:22px 0;padding:14px 22px;border-radius:6px;background:#176b43;color:white;text-decoration:none;font-weight:700">Open SecurePath Shield</a></div>`,
  });
}
...
export async function sendTransferVerificationEmail({
  ...
  await transporter.sendMail({
    ...
    html: `<div style="font-family:Arial,sans-serif;max-width:540px;margin:auto;padding:32px;color:#17211b"><div style="font-size:13px;font-weight:700;letter-spacing:3px;color:#176b43">SECUREPATH SHIELD</div><h1 style="font-size:27px;margin:28px 0 12px">${subject}</h1><p style="color:#66736b;line-height:1.7">${message}</p><a href="${supportUrl}" style="display:inline-block;margin:22px 0;padding:14px 22px;border-radius:6px;background:#176b43;color:white;text-decoration:none;font-weight:700">Contact customer support</a></div>`,
  });
}
