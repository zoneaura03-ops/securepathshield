...
const cards = [
  [Users, "Total customers", "users", "/admin/users", "from-gold-500 to-gold-700"],
  [Activity, "Pending transfers", "transfers", "/admin/transactions", "from-slate-700 to-slate-900"],
  [Landmark, "Deposit reviews", "deposits", "/admin/deposits", "from-gold-500 to-gold-700"],
  ...
  return (
    <div className="mx-auto max-w-[1500px]">
      ...
      <section className="mt-7 grid gap-6 xl:grid-cols-[1.35fr_.65fr]">
        <div className="rounded-3xl border border-[#dfe5ef] bg-white p-6 shadow-[0_12px_35px_rgba(10,23,40,.05)] sm:p-7">
        ...
        <div className="rounded-3xl bg-gradient-to-b from-[#0a1728] to-[#0a1728] p-6 text-white shadow-[0_18px_50px_rgba(10,23,40,.16)] sm:p-7">
          <span className="grid size-12 place-items-center rounded-2xl bg-gold-400/15 text-gold-300"><ShieldCheck size={23} /></span><p className="mt-6 text-[10px] font-bold uppercase tracking-[.18em] text-gold-300">Governance status</p><h2 className="mt-2 text-2xl">Protected and accountable</h2><p className="mt-3 text-sm leading-6 text-white/55">Every approval, decline, balance adjustment, and account change is attributed to the signed-in administrator.</p>
          <div className="mt-6 space-y-3 border-t border-white/10 pt-5">{["Administrator session active", "Audit logging enabled", "Approval controls operational"].map((item) => <div key={item} className="flex items-center gap-2 text-xs text-white/70"><CheckCircle2 size={15} className="text-gold-300" />{item}</div>)}</div>
          <Link href="/admin/audit" className="mt-7 inline-flex items-center gap-2 text-xs font-semibold text-gold-300">Review audit history <ArrowUpRight size={14} /></Link>
        </div>
      </section>
    </div>
  );
